...
class ReloaderLoop(object):
    ...
    def log_reload(self, filename):
        ...
        _log('info','* Detected changes in: %r, reloading' % filename)
...
def run_with_reloader(main_func, extra_files, *args, **kwargs):
    ...
    import signal
    reloader = reloader_loops[reloader_type](extra_files, interval_time)
    signal.signal(signal.SIGTERM, lambda *args: sys.exit(0))
    
    try:
        if os.environ.get('RELOADER') == 'true':
            if extra_files:
                _log('info', '* Extra files list:{0}'.format(extra_files))
